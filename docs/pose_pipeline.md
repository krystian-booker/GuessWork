...
| RIO FPGA clock | controller `RobotController.getFPGATime()` µs, carried in CAN chassis-speeds frames | `gw::RioClockSync` inside TeensyManager (windowed-min + drift fit; resets on controller reboot) — `src/core/rio_clock_sync.hpp` |
| Host steady clock | `std::chrono::steady_clock`, used only for arrival stamps and rate windows | `gw::fusion::TeensyNowEstimator` (EMA of measurement-stamp − host-arrival) supplies "Teensy-now" for output extrapolation |

Rules:
- **Estimation math never uses host clocks.** Frames are re-stamped from
  trigger pulses (`Frame::camera_ts_ns`), IMU samples are stamped on the
  Teensy before transit, chassis speeds carry the mapped RIO sample time
  (arrival-stamp fallback when `RioClockSync` is unhealthy).
- `TeensyNowEstimator` is fed by the fusion supervisor's **odom drainer**
  ...
  survives CAN-odom death. The tag feed's ~15–40 ms detect latency biases the
...
Target: **trigger pulse → pose on CAN p95 < 50 ms** (`pose_staleness`).
...
| `pose_staleness` | `teensy_now − newest state t_ns` at each CAN send, pre-clamp (signed; slightly negative possible with the tag-biased estimator) | **the headline end-to-end number** — extrapolation covers exactly this gap |
...
| CAN odom dead | `no_odom` | tags + VIO; states bridge on constant-velocity factors; teensy_now stays alive via the tag feed | barely changes | chassis-speeds path is the RIO's own — it already knows |
...
| Teensy unplugged | (any) | no CAN path at all; `send_pose` errors counted | — | counter freezes |
...
- `src/core/rio_clock_sync.hpp` — RIO↔Teensy mapping + reset rules.
...
- `docs/can-protocol.md` ↔ `firmware/src/can_payloads.h` — the CAN wire
  contract.
