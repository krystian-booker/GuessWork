...
// enabled by -DUSB_DUAL_SERIAL). One-way, Teensy → host. The ASCII command
// protocol on `Serial` is untouched — see serial_proto.h.
// telemetry_payloads.h is the freestanding (no Arduino.h) twin of this
// framing that the host test suite compiles directly.
//
...
//   0x02 HEARTBEAT  payload (17 bytes) = t_us u64,
//                   flags u8 (bit0: imu_ok),
