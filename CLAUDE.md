...
`RosbagWriter` is a minimal in-app ROS1 v2.0 bag writer: single `sensor_msgs/Image` topic (Mono8, uncompressed), no ROS install required on the host (see `src/consumer/rosbag_writer.cpp` for the record-framing details). `RosbagRecordingConsumer` is the `IConsumer` (lives in `gw_consumer`) that drives it: pull thread copies Mono8 out of the IOSurface and enqueues to a bounded queue; a single writer thread drains the queue into the bag. `CalibrationSupervisor` owns at most one consumer per camera, subscribed to that camera's `FrameChannel` via `CameraSupervisor::frame_channel_for(id)`. The bag layout is forward-compatible with IMU (`/imu0` topic) and stereo extrinsics (additional `/camN/image_raw` topics).
